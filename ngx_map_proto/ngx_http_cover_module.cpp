...
#include "../ngx_plan_proto/ngx_plan_proto.h"
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include <time.h>
...
static char *
ngx_http_cover(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);

static ngx_int_t ngx_http_cover_handler(ngx_http_request_t *r);

}
using namespace std;

static ngx_command_t ngx_http_cover_commands[] =
        {
...
static ngx_http_module_t ngx_http_cover_module_ctx =
        {
...
ngx_module_t ngx_http_cover_module =
        {
                NGX_MODULE_V1,
                &ngx_http_cover_module_ctx,           /* module context */
                ngx_http_cover_commands,              /* module directives */
...
//实现的ngx_http_mytest_handler方法处理这个请求
    clcf->handler = ngx_http_cover_handler;
...
ngx_int_t pack_cover(ngx_plan_t *plan, ngx_str_t *output, ngx_pool_t *pool, FILE *file) {
    FILE *fout;
...
static ngx_int_t ngx_http_cover_handler(ngx_http_request_t *r) {
    ...
    FILE *fout;
    fout = fopen("/tmp/nginx_cover_log", "a+tc");
    fprintf(fout, "m->start process!\n");


    fflush(fout);
    NS_Service::Client< std::vector<sgbot::Pose2D> >* plan_cli = new NS_Service::Client<std::vector<sgbot::Pose2D> >("COVER_VIZ");
    std::vector<sgbot::Pose2D> plan_vec;
    if(!plan_cli->call(plan_vec)){
        fprintf(fout, "call plan failed\n");
        return -1;
    }
    fprintf(fout,"get plan_vec size = %d\n",plan_vec.size() );
    ...
    for(int i = 0; i < plan_vec.size();++i){
        ...
        ngx_point__set_x(point, plan_vec[i].x() );
        ngx_point__set_y(point, plan_vec[i].y() );
        fprintf(fout, "add x = %d,y = %d\n", plan_vec[i].x(),plan_vec[i].y());
    }
    fprintf(fout, "construct_cover\n");


    ngx_str_t str = ngx_null_string;
    ngx_str_t *output = &str;
    fprintf(fout, "pack cover\n");
    fflush(fout);
    pack_cover(plan, output, r->pool, fout);
